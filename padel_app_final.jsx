...
function buildSignupMessage({ week, players, settings }) {
  ...
  if (week.bookers?.length > 0 && players) {
    const names = week.bookers.map(id => players.find(p => p.id === id)?.name).filter(Boolean);
    if (names.length > 0) {
      lines.push('');
      lines.push(`Booked by ${names.join(', ')} — ${names.length > 1 ? 'they\'re' : 'they\'re'} auto-entered. Thanks!`);
    }
  }
  lines.push('');
...
export default function PadelApp() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      if (allocated) save({ ...appData, currentWeek: allocated });
    }, ms);
  ...
  async function loadData() {
    try {
      ...
      if (!app) {
        app = { players: [], currentWeek: null, history: [], settings: { ...DEFAULT_SETTINGS }, weekCounter: 0, setupComplete: false };
      } else {
  ...
  async function startNewWeek({ deadline, gameAt, bookers = [] }) {
    ...
    save({ ...appData, currentWeek: week, weekCounter: week.number });
    setModal(null);
  }
  ...
  function toggleSignup(playerId) {
    ...
    save({ ...appData, currentWeek: { ...appData.currentWeek, signups } });
  }
  ...
  function joinLateAsReserve(playerId) {
    ...
    save({ ...appData, currentWeek: { ...week, signups } });
  }
  ...
  function editCurrentWeek({ deadline, gameAt }) {
    if (!appData.currentWeek) return;
    const week = { ...appData.currentWeek, deadline: deadline || null, gameAt: gameAt || null };
    save({ ...appData, currentWeek: week });
    setModal(null);
  }

  function markLineupShared() {
    if (!appData.currentWeek) return;
    if (appData.currentWeek.lineupShared) return;
    save({ ...appData, currentWeek: { ...appData.currentWeek, lineupShared: true } });
  }
  ...
  async function reopenSignups() {
    ...
    save({ ...appData, currentWeek: { ...week, status: 'open', signups: newSignups, allocatedAt: null } });
  }
  ...
  async function dropOutSelf(playerId) {
    ...
    const ok = await showConfirm({
      ...
      body: isStarter
        ...
        : 'You\'re on the reserve list. Dropping out just removes you — no penalty.',
      confirmLabel: 'Drop out',
      danger: isStarter && isLate,
    });
    if (!ok) return;
    ...
    if (myInfo.slot === 'reserve') {
      ...
      save({ ...appData, currentWeek: { ...week, signups: newSignups } });
      return;
    }
    ...
    const candidates = Object.entries(week.signups)
    ...
    const droppedName = appData.players.find(p => p.id === playerId)?.name;
    const dropper = appData.players.find(p => p.id === playerId);
    if (promotedName && dropper?.isOrganiser) {
  ...
  async function loadDemoData() {
    if (appData.players.length > 0) {
      const ok = await showConfirm({
        ...
        body: 'This wipes your existing players and history and loads 19 demo players (PIN 1234 for everyone).',
        confirmLabel: 'Load demo',
      });
      if (!ok) return;
    }
    const names = ['Eoin', 'Sarah', 'Marcus', 'Aisling', 'Rohan', 'Pia', 'Tom', 'Niamh', 'Diego', 'Yuki', 'Liam', 'Maeve', 'Jordan', 'Priya', 'Sven', 'Cara', 'Femi', 'Kira', 'Alex'];
    const players = [];
    for (let i = 0; i < names.length; i++) {
      const id = 'p_demo_' + i + '_' + Math.random().toString(36).slice(2, 5);
      const pinHash = await hashPin('1234', id);
      players.push({
        id, name: names[i], points: Math.floor(Math.random() * 4),
        createdAt: Date.now() - i * 1000,
        pinHash, claimed: true, isOrganiser: i === 0,
      });
    }
    const settings = { ...DEFAULT_SETTINGS, shareUrl: typeof window !== 'undefined' ? window.location.href : '' };
    await save({ players, currentWeek: null, history: [], settings, weekCounter: 0, setupComplete: true });
    await setUser(players[0].id);
  }
  ...
  async function resetAll() {
    ...
    if (!reallyOk) return;
    save({ players: [], currentWeek: null, history: [], settings: { ...DEFAULT_SETTINGS }, weekCounter: 0, setupComplete: false });
    clearUser();
  }
  ...
  return (
    <div style={{ background: styles.bg, color: styles.ink, fontFamily: FONT_SANS, minHeight: '100vh', WebkitFontSmoothing: 'antialiased' }}>
      <div className="max-w-2xl mx-auto px-5 pb-32">
        ...
        {!appData.setupComplete && (
          <FirstTimeSetup onCreate={createFirstOrganiser} onLoadDemo={loadDemoData} />
        )}

        {appData.setupComplete && (
          <>
            <nav className="flex gap-6 mt-8 mb-8 border-b" style={{ borderColor: styles.line }}>
              {[['week', 'This week'], ['roster', 'Roster'], ['history', 'History']].map(([k, label]) => (
                <button
                  key={k}
                  onClick={() => setView(k)}
                  style={{
...
function FirstTimeSetup({ onCreate, onLoadDemo }) {
    ...
    return (
      <div style={{ borderColor: styles.line, background: styles.surface }} className="border rounded-lg p-8 mt-8">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={onLoadDemo} style={{ borderColor: styles.line, color: styles.ink }} className="border px-5 py-2.5 rounded-md text-sm hover:bg-zinc-50">
            Load demo (PIN: 1234)
          </button>
        </div>
      </div>
...
function WeekView({ appData, currentUser, isOrganiser, onStartWeek, onEditWeek, onToggleSignup, onAllocate, onReopen, onWrapUp, onPromptLogin, onDropOut, onJoinLate, onMarkLineupShared }) {
  const week = appData.currentWeek;
      ...
      <div>
        ...
        <div className="space-y-1">
          ...
          {signedUpPlayers.map((p, i) => (
            <PlayerRow key={p.id} player={p} index={i} highlight={i < courtSize} subtle={i >= courtSize} isMe={p.id === currentUser?.id} isBooker={bookerSet.has(p.id)} />
          ))}
        </div>
    ...
    <div>
      ...
      <div className="space-y-1 mb-6">
        {starters.map(id => {
          ...
          return p && <PlayerRow key={id} player={p} highlight isMe={p.id === currentUser?.id} promoted={info.promoted} isBooker={info.isBooker} />;
        })}
      </div>
        ...
        <>
          ...
          <div className="space-y-1 mb-6">
            {reserves.map((id, i) => {
              const p = appData.players.find(pp => pp.id === id);
              const info = week.signups[id];
              return p && <PlayerRow key={id} player={p} subtle isMe={p.id === currentUser?.id} reserveIndex={i + 1} isBooker={info.isBooker} />;
            })}
          </div>
        </>
      )}
...
function PlayerRow({ player, index, highlight, subtle, isMe, promoted, reserveIndex, isBooker }) {
  return (
    <div
      style={{
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <span style={{ fontSize: 14, fontWeight: isMe ? 700 : 500, color: styles.ink }}>
          {player.name}
...
function RosterView({ roster, currentUserId, isOrganiser, onAdd, onRemove, onResetPin, onToggleOrganiser, onResetPoints, onStartFresh, onReset }) {
  return (
    <div>
