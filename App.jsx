// localStorage shim so window.storage works outside Claude artifacts
// (Claude's storage API doesn't exist in regular browsers — fall back to localStorage)
if (typeof window !== 'undefined' && !window.storage) {
  const PREFIX = '__fitztues__:';
  window.storage = {
    async get(key) {
      try {
    ...
    async set(key, value) {
      try { localStorage.setItem(PREFIX + key, value); return { value }; }
      catch (e) { return null; }
    },
    async delete(key) {
      try { localStorage.removeItem(PREFIX + key); return { deleted: true }; }
      catch (e) { return null; }
    },
    async list() {
      try {
...
function formatAuditEntry(e, players) {
  ...
  switch (e.type) {
    ...
    case 'late_signup':
      return `${actor} signed up after the deadline — added to reserves, no +1 eligibility`;
    case 'allocation_ran': {
    ...
    case 'dropped_out': {
      ...
      return `${actor} dropped out from ${fromSlot} (${tag})`;
    }
...
export default function PadelApp() {
  ...
  function joinLateAsReserve(playerId) {
    ...
    next = appendAudit(next, { type: 'late_signup', actor: playerId });
    save(next);
  }
  ...
  async function dropOutSelf(playerId) {
    ...
    next = appendAudit(next, {
      type: 'dropped_out',
      actor: playerId,
      meta: { fromSlot: 'starter', window: isLate ? 'late' : 'early', cancelHours: appData.settings.cancelHours || 48 },
    });
    if (promotedId) {
      next = appendAudit(next, {
