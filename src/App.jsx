          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p style={{ color: styles.inkMuted, fontSize: 13 }} className="mt-2 leading-relaxed">
                Fair weekly signups: priority points decide who plays.{' '}
                <button
...
function WeatherStrip({ gameAt }) {
  ...
  useEffect(() => {
    ...
    fetch(url)
      ...
      .then(data => {
        ...
        // Find the closest forecast hour to the game time
        const gameTs = gameAt;
        ...
        for (let i = 0; i < times.length; i++) {
          ...
          const diff = Math.abs(t - gameTs);
          if (diff < bestDiff) { bestDiff = diff; bestIdx = i; }
        }
        // Bail if the closest hour is more than 90 minutes off (means out of range)
        if (bestIdx === -1 || bestDiff > 90 * 60 * 1000) {
          setState({ status: 'out_of_range' });
          return;
        }
        setState({
  ...
  if (state.status === 'loading' || state.status === 'error' || state.status === 'out_of_range') return null;
