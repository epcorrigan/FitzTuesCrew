...
function formatAuditEntry(e, players) {
  ...
  switch (e.type) {
    ...
    case 'late_signup': {
      const by = e.meta.byOrganiser ? ' (added by organiser)' : '';
      if (e.meta.fillingGap) {
        return `${actor} added late${by} — filled a vacant starter slot. Earns +1 if they play.`;
      }
      return `${actor} signed up after the deadline${by} — added to reserves. Earns +1 if bumped to starter and plays; nothing if not bumped.`;
    }
    case 'allocation_ran': {
...
function buildSignupMessage({ week, players, settings, forecast }) {
  ...
  if (week.deadline) lines.push(`Sign up by: ${fmtDateTime(week.deadline)}`);
  if (forecast) {
    const parts = [];
    const desc = describeWeatherCode(forecast.code);
    if (desc.label) parts.push(`${desc.emoji} ${desc.label}`);
    parts.push(`${forecast.temp}°C`);
    if (forecast.rainPct !== null && forecast.rainPct !== undefined) parts.push(`${forecast.rainPct}% rain`);
    lines.push(`Forecast: ${parts.join(' · ')}`);
  }
  ...
  if (week.deadline) {
    lines.push('');
    lines.push('Please sign up by the deadline so the lineup can be finalised. Late signups still welcome.');
  }
  lines.push('');
...
export default function PadelApp() {
  ...
  function joinLateAsReserve(playerId) {
    ...
    // If the court is short (fewer starters than courtSize), late signups fill
    // the gap directly rather than sitting on the reserve list. They're tagged
    // as having come from the reserve queue (originalSlot: 'reserve' + promoted
    // flag) so the points logic treats them like a promoted reserve — earning
    // +1 if they play, since they did the group a favour by stepping in.
    const courtSize = appData.settings.courtSize || 12;
    const currentStarters = Object.values(week.signups).filter(s => s.slot === 'starter').length;
    const fillingGap = currentStarters < courtSize;

    const newSignup = {
      slot: fillingGap ? 'starter' : 'reserve',
      originalSlot: 'reserve',
      outcome: null,
      signupCount: Object.keys(week.signups).length + 1,
      signupTime: Date.now(),
      ...(fillingGap && { promoted: true }),
    };
    ...
    next = appendAudit(next, {
      type: 'late_signup',
      actor: playerId,
      meta: { byOrganiser: currentUserId !== playerId, fillingGap },
    });
    save(next);
  }
  ...
  return (
    <div style={{ background: styles.bg, color: styles.ink, fontFamily: FONT_SANS, minHeight: '100vh', WebkitFontSmoothing: 'antialiased' }}>
      <div className="max-w-2xl mx-auto px-5 pb-32">
        <header className="pt-10 pb-6 border-b" style={{ borderColor: styles.line }}>
          <div className="flex items-start justify-between">
            <div>
              ...
              <p style={{ color: styles.inkMuted, fontSize: 13 }} className="mt-2 leading-relaxed">
                Places allocated using a points method each week.{' '}
                <button
                  onClick={() => setModal({ type: 'help' })}
                  style={{ color: styles.ink, textDecoration: 'underline', textDecorationColor: styles.line, textUnderlineOffset: 3 }}
                  className="hover:decoration-zinc-500"
                >
                  How it works
                </button>
              </p>
        ...
        {appData.setupComplete && !currentUser && (
          <div style={{ borderColor: styles.line, background: styles.surface }} className="border rounded-lg p-8 mt-8 text-center">
            <div style={{ fontWeight: 700, fontSize: 22, color: styles.inkStrong, letterSpacing: '-0.02em' }} className="mb-3">
              Sign in to view
            </div>
            <p style={{ color: styles.inkMuted }} className="mb-6 text-sm leading-relaxed max-w-md mx-auto">
              Roster, weekly lineups, and history are visible to signed-in members of the group only. Pick your name and enter your PIN to continue.
            </p>
            <button
              onClick={() => setModal({ type: 'switchUser' })}
              style={{ background: styles.ink, color: styles.bg }}
            ...
            <p style={{ color: styles.inkSubtle, fontSize: 11 }} className="mt-6 leading-relaxed max-w-md mx-auto">
              Don't have an account yet? Ask an organiser to add you to the roster, then come back here with the temporary PIN they'll share.
            </p>
          </div>
        )}
        ...
        {appData.setupComplete && currentUser && (
          <>
            ...
            {view === 'stats' && isOrganiser && (
              <StatsView history={appData.history} players={appData.players} />
            )}
...
function SwitchUserModal({ roster, currentUserId, onClose, onAuthenticate, onClaim }) {
  ...
  // Privacy: don't reveal the roster until the user types something. Outsiders
  // who land on the URL shouldn't be able to browse the full membership list.
  // We require at least 2 characters before showing matches to avoid trivially
  // dumping the whole roster (a single "a" or "e" would reveal too much).
  const trimmed = query.trim().toLowerCase();
  const matches = trimmed.length >= 2
    ? roster.filter(p => p.name.toLowerCase().includes(trimmed)).slice(0, 25)
    : [];
  ...
  return (
    <Modal onClose={onClose} title={step === 'pick' ? 'Sign in' : selected?.name} wide={step === 'pick'}>
      {step === 'pick' && (
        <div>
          <p style={{ color: styles.inkMuted }} className="text-sm mb-3 leading-relaxed">
            Type your name to find your account.
          </p>
          <input
            autoFocus
            type="text"
          ...
          {trimmed.length === 0 && (
            <p style={{ color: styles.inkSubtle, fontSize: 12 }} className="text-center py-6 leading-relaxed">
              Start typing your name above.
            </p>
          )}
          ...
          {trimmed.length >= 2 && matches.length === 0 && (
            <div style={{ color: styles.inkMuted, fontSize: 13 }} className="text-center py-6 leading-relaxed">
              <p className="mb-1">No matches.</p>
              <p style={{ fontSize: 12 }}>If you should be in the group, ask an organiser to add you.</p>
            </div>
          )}
    ...
    <Modal onClose={onClose} title="How it works" wide>
      <div className="space-y-5 text-sm" style={{ color: styles.ink }}>
        <div>
          ...
          <p style={{ color: styles.inkMuted }} className="text-xs mt-2 leading-relaxed">Higher points get first pick each week. Where points are equal, places are allocated randomly. The allocation runs automatically at the signup deadline. Late additions are still welcome — they fill any empty starter slot, otherwise they join the reserves. Either way, they earn +1 if they play.</p>
        </div>
        <div>
        ...
        <div>
          ...
          <p style={{ color: styles.inkMuted }} className="leading-relaxed">You can drop out any time. Before allocation, just tap your signed-up confirmation to remove yourself — no penalty. After allocation, tap "I can't make it" on the week page; the top-priority reserve is auto-promoted instantly. If you cancel within {cancelHours} hours of the game it's a late cancel (−2 points). The points rules apply equally to everyone, organisers included.</p>
        </div>
        <div>
...
function WeekView({ appData, currentUser, isOrganiser, onStartWeek, onEditWeek, onToggleSignup, onAllocate, onReopen, onWrapUp, onPromptLogin, onDropOut, onJoinLate, onMarkLineupShared, onRemovePlayer }) {
  ...
  // Pre-fetch the forecast so the WhatsApp share handler can run synchronously
  // (mobile browsers block window.open inside an async chain).
  const [forecast, setForecast] = useState(null);
  useEffect(() => {
    let cancelled = false;
    fetchForecast(week.gameAt).then(f => { if (!cancelled) setForecast(f); });
    return () => { cancelled = true; };
  }, [week.gameAt]);

  const shareSignup = () => {
    window.open(whatsappUrl(buildSignupMessage({ week, players: appData.players, settings, forecast })), '_blank');
  };
  const shareLineup = () => {
  ...
  if (week.status === 'open') {
    ...
    return (
      <div>
        ...
        {week.deadline && (
          <div style={{ background: styles.surface, borderColor: styles.line }} className="border rounded-md p-3 mb-3">
            <p style={{ color: styles.inkMuted, fontSize: 12 }} className="leading-relaxed">
              Please sign up before the deadline so the lineup can be finalised. Late signups still welcome.
            </p>
          </div>
        )}
        ...
        {currentUser ? (
          meSignedUp ? (
            // Already signed up: quieter confirmation strip, not a hero CTA
            <button
              onClick={() => onToggleSignup(currentUser.id)}
              style={{ background: styles.surface, borderColor: styles.line, color: styles.ink }}
              className="w-full border rounded-md p-3 mb-3 flex items-center justify-between transition-colors hover:bg-zinc-100"
            >
              <div className="flex items-center gap-2.5">
                <span style={{ width: 24, height: 24, borderRadius: 999, background: '#16a34a', color: 'white', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700 }}>✓</span>
                <span>
                  <div style={{ fontWeight: 600, fontSize: 14 }}>You're signed up</div>
                  <div style={{ fontFamily: FONT_MONO, fontSize: 10, color: styles.inkMuted, marginTop: 1 }}>
                    Tap to drop out · priority {currentUser.points >= 0 ? '+' : ''}{currentUser.points}
                  </div>
                </span>
              </div>
            </button>
          ) : (
            // Not signed up: hero CTA — big, dark, unmissable
            <button
              onClick={() => onToggleSignup(currentUser.id)}
              style={{ background: styles.ink, color: styles.bg }}
              className="w-full rounded-md py-5 px-5 mb-3 flex items-center justify-between transition-transform hover:translate-y-[-1px] active:translate-y-0"
            >
              <span style={{ fontWeight: 700, fontSize: 19, letterSpacing: '-0.01em' }}>
                Sign me up
              </span>
              <span style={{ fontFamily: FONT_MONO, fontSize: 11, opacity: 0.65 }}>
                priority {currentUser.points >= 0 ? '+' : ''}{currentUser.points} →
              </span>
            </button>
          )
        ) : (
          <button onClick={onPromptLogin} style={{ background: styles.ink, color: styles.bg }} className="w-full rounded-md p-3.5 mb-3 text-center font-medium text-sm">
            Sign in to sign up
          </button>
        )}

        {isOrganiser && (
          <button onClick={shareSignup} style={{ background: styles.whatsapp, color: 'white' }} className="w-full rounded-md p-2.5 mb-6 flex items-center justify-center gap-2 font-medium text-sm">
  ...
  return (
    <div>
      ...
      {starters.length < courtSize && (
        <div style={{ background: '#fef3c7', borderColor: '#f59e0b' }} className="border rounded-md p-3.5 mb-3">
          ...
          <p style={{ color: '#92400e' }} className="text-xs mb-3 leading-relaxed">
            The lineup is short. Anyone available to play?
          </p>
          {isOrganiser && (
            <button
              onClick={() => {
      ...
      {currentUser && !myInfo && (() => {
        const courtIsShort = starters.length < courtSize;
        return (
          <button
          ...
          >
            {courtIsShort ? 'Sign me up — fill the empty slot' : 'Add me to the reserve list'}
            <div style={{ fontFamily: FONT_MONO, fontSize: 10, color: styles.inkMuted, fontWeight: 400 }} className="mt-1 normal-case">
              {courtIsShort
                ? 'Court is short — you go straight in. +1 if you play.'
                : 'Lineup is full — you join the reserves. +1 only if you get bumped and play.'
              }
            </div>
          </button>
        );
      ...
      {isOrganiser && (() => {
        ...
        return (
          <div className="mb-6">
            <SectionLabel>Add late signup · organiser only</SectionLabel>
            <p style={{ color: styles.inkMuted }} className="text-xs mb-2 leading-relaxed">
              {courtIsShort
                ? `Tap to add someone — they'll fill an empty starter slot directly. If they play, they earn +1 (treated as a sub stepping in).`
                : `Tap to add someone — they'll join the reserve list. If they get bumped to starter and play, they earn +1. If they're not bumped, they don't earn the standard "didn't get a spot" bonus (that's reserved for on-time signups).`
              }
            </p>
            <div className="flex flex-wrap gap-1.5">
              {notSignedUp.map(p => (
...
// Open-Meteo: free, no API key, hourly forecast for any lat/lon.
// Forecasts are reliable up to ~7 days out; beyond that the API doesn't return data
// for our requested hour and the strip just won't render.
const WEATHER_LAT = 53.327; // Ranelagh, Dublin
const WEATHER_LON = -6.247;

// Map WMO weather codes to a readable label + emoji.
// Reference: https://open-meteo.com/en/docs (search "Weather variable WMO code")
function describeWeatherCode(code) {
  if (code === 0) return { emoji: '☀️', label: 'Clear' };
  if (code === 1) return { emoji: '🌤', label: 'Mostly clear' };
  if (code === 2) return { emoji: '⛅', label: 'Partly cloudy' };
  if (code === 3) return { emoji: '☁️', label: 'Overcast' };
  if ([45, 48].includes(code)) return { emoji: '🌫', label: 'Foggy' };
  if ([51, 53, 55].includes(code)) return { emoji: '🌦', label: 'Drizzle' };
  if ([56, 57].includes(code)) return { emoji: '🌧', label: 'Freezing drizzle' };
  if ([61, 63, 65].includes(code)) return { emoji: '🌧', label: 'Rain' };
  if ([66, 67].includes(code)) return { emoji: '🌧', label: 'Freezing rain' };
  if ([71, 73, 75, 77].includes(code)) return { emoji: '🌨', label: 'Snow' };
  if ([80, 81, 82].includes(code)) return { emoji: '🌧', label: 'Showers' };
  if ([85, 86].includes(code)) return { emoji: '🌨', label: 'Snow showers' };
  if (code === 95) return { emoji: '⛈', label: 'Thunderstorm' };
  if ([96, 99].includes(code)) return { emoji: '⛈', label: 'Thunderstorm w/ hail' };
  return { emoji: '·', label: '' };
}

// Shared forecast fetcher — used by both the inline WeatherStrip and the WhatsApp
// share flow. Returns { temp, rainPct, code } or null on failure / out of range.
async function fetchForecast(gameAt) {
  if (!gameAt) return null;
  const daysAhead = (gameAt - Date.now()) / (1000 * 60 * 60 * 24);
  if (daysAhead < -0.1 || daysAhead > 14) return null;
  try {
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${WEATHER_LAT}&longitude=${WEATHER_LON}&hourly=temperature_2m,precipitation_probability,weathercode&timezone=Europe%2FDublin&forecast_days=14`;
...
function WeatherStrip({ gameAt }) {
  const [state, setState] = useState({ status: 'loading' });

  useEffect(() => {
    if (!gameAt) return;
    let cancelled = false;
    fetchForecast(gameAt).then(result => {
      if (cancelled) return;
      if (!result) setState({ status: 'unavailable' });
      else setState({ status: 'ok', ...result });
    });
    return () => { cancelled = true; };
  }, [gameAt]);
  ...
  const { emoji, label } = describeWeatherCode(state.code);
  return (
    <div style={{ fontFamily: FONT_MONO, fontSize: 11, color: styles.inkMuted }} className="flex items-center gap-2">
      <span>Forecast:</span>
      <span style={{ fontFamily: 'inherit' }}>{emoji}</span>
...
function WeekHeader({ eyebrow, number, total, label, subRight, gameAt, deadline }) {
  return (
    <div style={{ borderColor: styles.line }} className="border rounded-lg p-5 mb-3">
      ...
      {gameAt && (
        <div
          style={{ fontWeight: 800, fontSize: 32, lineHeight: 1.1, letterSpacing: '-0.025em', color: styles.inkStrong }}
          className="mb-2"
        >
          {fmtDateTime(gameAt)}
        </div>
      ...
      {(deadline || gameAt) && (
        <div className="mt-3 pt-3 border-t space-y-1" style={{ borderColor: styles.line, fontFamily: FONT_MONO, fontSize: 11, color: styles.inkMuted }}>
          {deadline && <div>Sign up by: {fmtDateTime(deadline)}{deadlineRel(deadline) && <span style={{ color: styles.ink, marginLeft: 6 }}>· {deadlineRel(deadline)}</span>}</div>}
          {gameAt && <WeatherStrip gameAt={gameAt} />}
        </div>
      )}
    </div>
  );
...
function AuditView({ audit, players, currentWeekNumber }) {
  ...
  return (
    <div>
      <p style={{ color: styles.inkMuted }} className="text-xs mb-4 leading-relaxed">
        Every action that affects a week — signups, drop-outs, allocations, edits — is logged here. Visible to everyone in the group for transparency.
      </p>
...
// Stats: aggregate per-player counts across all completed weeks.
// Definitions:
//   - Played  = outcome === 'played' (regardless of starter or promoted reserve)
//   - Signed up = appeared in any week's signups (any slot or outcome)
//   - Late cancels = outcome === 'cancelled_late'
//   - No-shows = outcome === 'no_show'
function StatsView({ history, players }) {
  ...
  if (!history || history.length === 0) {
    return (
      <div>
        <p style={{ color: styles.inkMuted }} className="text-sm mb-4 leading-relaxed">
          Player activity across completed weeks. Visible to organisers only.
        </p>
        <div style={{ color: styles.inkMuted }} className="text-sm text-center py-8 border rounded-md" >
          No completed weeks yet — once you wrap up a week, stats will appear here.
        </div>
      </div>
    );
  }
  ...
  const stats = {};
  players.forEach(p => {
    stats[p.id] = { id: p.id, name: p.name, played: 0, signedUp: 0, lateCancels: 0, noShows: 0 };
  });
  history.forEach(week => {
    Object.entries(week.signups || {}).forEach(([id, info]) => {
      if (!stats[id]) return; // player removed from roster — skip silently
      stats[id].signedUp += 1;
      if (info.outcome === 'played') stats[id].played += 1;
      else if (info.outcome === 'cancelled_late') stats[id].lateCancels += 1;
      else if (info.outcome === 'no_show') stats[id].noShows += 1;
    });
  });

  // Filter to only players who participated at all (avoid showing 70 rows of zeros)
  const rows = Object.values(stats)
    .filter(r => r.signedUp > 0)
    .sort((a, b) => {
      if (sortBy === 'name') return a.name.localeCompare(b.name);
      return (b[sortBy] || 0) - (a[sortBy] || 0) || a.name.localeCompare(b.name);
    });

  const totalWeeks = history.length;

  function HeaderCell({ field, label, align = 'right' }) {
    ...
    return (
      <button
        ...
        style={{
          ...
          fontSize: 10,
          color: active ? styles.ink : styles.inkMuted,
          fontWeight: active ? 700 : 500,
          letterSpacing: '0.1em',
          textAlign: align,
        }}
        className="uppercase hover:text-zinc-900"
      >
        {label}{active ? ' ↓' : ''}
      </button>
    );
  ...
  return (
    <div>
      <p style={{ color: styles.inkMuted }} className="text-sm mb-2 leading-relaxed">
        Player activity across {totalWeeks} completed week{totalWeeks === 1 ? '' : 's'}. Tap a column header to re-sort. Visible to organisers only.
      </p>

      <div style={{ borderColor: styles.line }} className="border rounded-md overflow-hidden mt-4">
        {/* Header row */}
        <div
          style={{ background: styles.surface, borderColor: styles.line }}
          className="grid border-b px-3 py-2 items-center"
          // 5 cols: name (flex), played, signups, late cancels, no-shows
          // Using grid-cols arbitrary so values align with rows below
        >
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 56px 56px 64px 64px', gap: 8, alignItems: 'center' }}>
            <HeaderCell field="name" label="Player" align="left" />
            <HeaderCell field="played" label="Played" />
            <HeaderCell field="signedUp" label="Signed up" />
            <HeaderCell field="lateCancels" label="Late cancel" />
            <HeaderCell field="noShows" label="No-show" />
          </div>
        </div>

        {/* Body rows */}
        {rows.length === 0 && (
          <div style={{ color: styles.inkMuted }} className="text-sm text-center py-6">
        ...
        {rows.map((r, i) => (
          <div
            key={r.id}
            style={{ borderColor: styles.line, background: i % 2 === 0 ? styles.bg : styles.surface }}
            className={`px-3 py-2 ${i < rows.length - 1 ? 'border-b' : ''}`}
          >
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 56px 56px 64px 64px', gap: 8, alignItems: 'center' }}>
              <span style={{ fontSize: 14, color: styles.ink }} className="truncate">{r.name}</span>
              <span style={{ fontFamily: FONT_MONO, fontSize: 13, color: styles.ink, fontWeight: 600, textAlign: 'right' }}>{r.played}</span>
              <span style={{ fontFamily: FONT_MONO, fontSize: 13, color: styles.inkMuted, textAlign: 'right' }}>{r.signedUp}</span>
              <span style={{ fontFamily: FONT_MONO, fontSize: 13, color: r.lateCancels > 0 ? styles.warn : styles.inkSubtle, textAlign: 'right' }}>{r.lateCancels}</span>
              <span style={{ fontFamily: FONT_MONO, fontSize: 13, color: r.noShows > 0 ? styles.warn : styles.inkSubtle, textAlign: 'right' }}>{r.noShows}</span>
            </div>
          </div>
        ))}
      </div>
