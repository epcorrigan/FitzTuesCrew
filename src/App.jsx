...
function buildSignupMessage({ week, players, settings }) {
  ...
  if (week.deadline) {
    lines.push('');
    lines.push('Please sign up before the deadline so the lineup can be finalised.');
  }
  lines.push('');
        ...
        {week.deadline && (
          <div style={{ background: styles.surface, borderColor: styles.line }} className="border rounded-md p-3 mb-3">
            <p style={{ color: styles.inkMuted, fontSize: 12 }} className="leading-relaxed">
              Please sign up before <span style={{ color: styles.ink, fontWeight: 600 }}>{fmtDateTime(week.deadline)}</span> so the lineup can be finalised.
            </p>
          </div>
        )}
